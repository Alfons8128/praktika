...
jednotlivé dílky se~na~záznamu intenzity projeví ostrým poklesem, který lze popsat funkcí ve~tvaru \cite{newton}
\begin{equation}
    \label{eq:meritko}
...
Ke zjištění poloměrů křivosti čoček byl použit digitální záznam obrazu z~mikroskopu a~následně 
...
a~\texttt{Newtonrings} jsou vykresleny na~obrázcích \ref{fig:fit_meritko} a~\ref{fig:fit_cocky}.

\textbf{TO DO Origin fity meritka a krouzku}
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{c|cc|cc}
        ...
        \midrule
        F50 & $neco$ & $neco$ & $30.06$ & $-172.0$ \\
        F100 & $neco$ & $neco$ & $60.02$ & $-353.30$ \\
        \bottomrule
    \end{tabular}
    \label{tab:polomery}
\end{table}
...
$\lambda_\mathrm{Na} = 589.3$ \cite{vlnovedelky} index lomu $n = 1.51673$ \cite{schott}.
...
\begin{equation}
    \label{eq:sigmaf}
    \sigma_f = \frac{n}{n-1} \frac{R_2 \cdot \left( n (R_2 - R_1) + (n-1) d \right) - 
    \textbf{TO DO}}{\left( n (R_2 - R_1) + (n-1) d \right)^2}
\end{equation}

\textbf{TO DO ohniskova vzdalenost}
...
Výrobce doporučuje provádět měření s~d~čárou Helia o~vlnové délkou $\lambda_d = \qty{587.6}{nm}$, pro~kterou
má optické sklo index lomu $n_\mathrm{d} = 1.51680$. Měření však probíhalo se~sodíkovou výbojkou
vyzařující na vlnové délce $\lambda_\mathrm{Na} = 589.3$ \cite{vlnovedelky} (spektrální čára D), 
které odpovídá nepatrně menší index lomu $n_\mathrm{D} = 1.51673$ \cite{schott}.
...
Newtonových kroužků určeny jako \textbf{TO DO nejistoty polomeru}
\vspace{-1mm}
\begin{itemize}
    \item čočka F50: $R_1 = 30.27(nejistota) \,\unit{mm}$, $R_2 = -172.38(nejistota) \,\unit{mm}$,
    \item čočka 100: $R_1 = 60.35(nejistota) \,\unit{mm}$, $R_2 = -354.4(nejistota) \,\unit{mm}$.
\end{itemize}

Z poloměrů křivosti a tlouštěk čoček byla nakonec vypočteny jejich ohniskové vzdálenosti \textbf{TO DO ohniskove vzdalenosti}
\vspace{-1mm}
\begin{itemize}
    \item čočka F50: $f = cislo \,\unit{mm}$,
    \item čočka F100: $f = cislo \,\unit{mm}$.
\end{itemize}
